...
export default function recommendRouter(prisma) {
  ...
  router.get('/top', async (req, res) => {
    ...
    try {
      ...
      return res.json({ items: dietProducts });
    } catch (err) {
      // Fallback: use existing isDiet flags/local scoring
      const dietProducts = products
        .filter(p => p.isDiet === true)
        .map(p => ({ ...p, score: scoreProduct(p) }))
        .sort((a, b) => b.score - a.score)
        .slice(0, limit);
      return res.json({ items: dietProducts, fallback: true });
    }
  });
